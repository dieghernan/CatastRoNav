...
Configure the cache directory used by CatastRoNav. Use
...
- overwrite:
  ...
  Logical. If `TRUE`, overwrites an existing `CATASTRONAV_CACHE_DIR`
  value already configured on your machine.
...
`catrnav_set_cache_dir()` invisibly returns a character string
containing the cache path. It is primarily called for its side effect.
...
#> ℹ /tmp/Rtmpj4964c/CatastRoNav

example_cache <- file.path(tempdir(), "example", "cache")
catrnav_set_cache_dir(example_cache)
#> ℹ CatastRoNav cache directory is /tmp/Rtmpj4964c/example/cache.
#> ℹ To reuse this cache directory in future sessions, set `install` to `TRUE`.

catrnav_detect_cache_dir()
#> ℹ /tmp/Rtmpj4964c/example/cache
#> [1] "/tmp/Rtmpj4964c/example/cache"

# Restore the initial cache.
catrnav_set_cache_dir(my_cache)
#> ℹ CatastRoNav cache directory is /tmp/Rtmpj4964c/CatastRoNav.
#> ℹ To reuse this cache directory in future sessions, set `install` to `TRUE`.
