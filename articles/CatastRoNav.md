...
2.  **WFS service:** Downloads vector objects for a selected bounding
    box.
3.  **WMS service:** Downloads georeferenced map images for different
    cadastral elements.

ATOM and WFS provide addresses, buildings and cadastral parcels as `sf`
objects from **sf**. WMS provides georeferenced images as `SpatRaster`
objects from **terra**.
...
by recreating a cadastral map of the surroundings of the [El Sadar
...
![Figure 1: El Sadar example](./sadar-1.png)

Figure 1: El Sadar example

### Thematic maps

We can also create thematic maps from attributes in the spatial objects.
This example visualizes Pamplona’s urban growth with **CatastRoNav**,
